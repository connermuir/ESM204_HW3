...
```{r}
# THIS WAS INCORRECT< DISREGARD FOR NOW 
...
#Now need to calculate when AD equals the supply curve 

# Supply curve -- *P = 0.0000186Q*
Qagg_tax <- map(price, demand_agg_tax) %>% 
  unlist()

agg_df_tax<- tibble(Qagg_tax = Qagg_tax, price = price)

# Note this is incorrect given the kink, but will get us the socially optimal levels since it is down the kink. 
demand_agg_tax_eq <- lm(price ~ Qagg_tax, data = agg_df_tax)
#extract_eq(model = demand_agg_tax_eq, use_coefs = TRUE, coef_digits = 5)
# 28.21529 - 0.00004(Q) = .0000186Q
#This gives Q* of 481,490 
# At this Q*, P* will be 

P_new <- 28.21529-0.00004*(481490)
# New price is $8.96
# Not sure this makes intuitive sense, price going down with the implementation of a tax since we are assuming AD is dropping and no supply impacts, so price should decrease technically, this is all quite misleading/opaque
